...
@defcomp doeclim begin
    ...
    function init(p, v, d)
        ...
        v.KT0[end] = 4 - 2 * sqrt(2.0)

        # First Order

        v.KTA1[end] = -8 * exp(-v.taubot / p.deltat) + 4 * sqrt(2.0) * exp(-0.5 * v.taubot / p.deltat)

        v.KTB1[end] = 4 * sqrt(pi * v.taubot / p.deltat) * (1 + erf(sqrt(0.5 * v.taubot / p.deltat)) - 2 * erf(sqrt(v.taubot / p.deltat)))

        # Second order

        v.KTA2[end] = 8 * exp(-4 * v.taubot / p.deltat) - 4 * sqrt(2.0) * exp(-2 * v.taubot / p.deltat)

        v.KTB2[end] = -8 * sqrt(pi * v.taubot / p.deltat) * (1 + erf(sqrt(2 * v.taubot / p.deltat)) - 2 * erf(2 * sqrt(v.taubot / p.deltat)))

        # Third Order

        v.KTA3[end] = -8 * exp(-9 * v.taubot / p.deltat) + 4 * sqrt(2.0) * exp(-4.5 * v.taubot / p.deltat)
        ...
        v.Cdoe[1, 1] = (1 / v.taucfl^2 + 1 / v.taukls^2 + 2 / v.taucfl / v.taukls + bsi / v.taukls / v.tauksl) * (p.deltat^2 / 12.0)
        v.Cdoe[1, 2] = (-bsi / v.taukls^2 - bsi / v.taucfl / v.taukls - bsi / v.taucfs / v.taukls - bsi^2 / v.taukls / v.tauksl) * (p.deltat^2 / 12.0)
        v.Cdoe[2, 1] = (-bsi / v.tauksl^2 - 1 / v.taucfs / v.tauksl - 1 / v.taucfl / v.tauksl - 1 / v.taukls / v.tauksl) * (p.deltat^2 / 12.0)
        v.Cdoe[2, 2] = (1 / v.taucfs^2 + bsi^2 / v.tauksl^2 + 2 * bsi / v.taucfs / v.tauksl + bsi / v.taukls / v.tauksl) * (p.deltat^2 / 12.0)

        # Matrices of difference equation system B*T(i+1) = Q(i) + A*T(i)
        # T = (TL,TO)
        # (Equation A.27, EK05, or Equations 2.3.24 and 2.3.27, TK07)
        v.Baux[1, 1] = 1.0 + p.deltat / (2 * v.taucfl) + p.deltat / (2 * v.taukls) + v.Cdoe[1, 1]
        v.Baux[1, 2] = -p.deltat / (2 * v.taukls) * bsi + v.Cdoe[1, 2]
        v.Baux[2, 1] = -p.deltat / (2 * v.tauksl) + v.Cdoe[2, 1]
        v.Baux[2, 2] = 1.0 + p.deltat / (2 * v.taucfs) + p.deltat / (2 * v.tauksl) * bsi + 2 * fso * sqrt(p.deltat / v.taudif) + v.Cdoe[2, 2]
    ...
    function run_timestep(p, v, d, n)
        ...
        if !is_first(n)
            ...
            QC1 = QC1 * p.deltat^2 / 12.0
            QC2 = QC2 * p.deltat^2 / 12.0
            # -------------------------- INITIAL CONDITIONS ------------------------
