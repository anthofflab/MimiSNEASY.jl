...
include("loadobs.jl")
include("../julia/sneasy.jl")

function sneasy_load_data()
	# load emissions data time series.
	df_emissions = readtable("../data/RCP85_EMISSIONS.csv")
	rename!(df_emissions, :YEARS, :year)

	df_forcings = readtable("../data/forcing_rcp85.txt", separator=' ')
...
function construct_log_post(endyear=2010; assim_temp=true, assim_ocheat=true, assim_co2inst=true, assim_co2ice=true, assim_co2ice=true)
...
#			llik.temp = logl.ar1(resid.temp, sigma.temp, rho.temp) # AR(1)
		...
		USE ENUMERATE TO MAKE THIS SMARTER#
			#There's probably a smarter way to do this.
			llik_co2ice = sum([logpdf(Normal(mean_co2ice[i], σ_co2ice), obs_co2ice[obs_co2ice_indiceswithdata][i]) for i=1:length(obs_co2ice_indiceswithdata)])
		end
...
#THESE TWO ARE THE SAME
		llik_ocflux = 0.
		if assim_ocflux
			llik_ocflux = sum([logpdf(Normal(atm_oc_flux[obs_ocflux_indiceswithdata][i], obs_ocflux_err[obs_ocflux_indiceswithdata][i]), obs_ocflux[obs_ocflux_indiceswithdata][i]) for i=1:length(obs_ocflux_indiceswithdata)])
		end
