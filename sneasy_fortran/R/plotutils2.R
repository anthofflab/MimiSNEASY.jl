...
plot.marginals <- function(chain, qlo=0.025, qhi=0.975, smoothing=1, truncate.kde=TRUE, stats=TRUE)
{
	N <- dim(chain)[1]; np <- dim(chain)[2]
	
	ncol <- floor(sqrt(np))
	nrow <- ceiling(np/ncol)
	par(mfrow=c(nrow,ncol), mar=c(3.5,3,2,0.5))
	...
	for(i in 1:np) {
		...
		plot.default(NA, xlim=range(x), ylim=c(0,max(y)), main=colnames(chain)[i], xlab="", ylab="", frame=FALSE, cex.axis=1.2, cex.main=1.4)
		if(truncate.kde) {
